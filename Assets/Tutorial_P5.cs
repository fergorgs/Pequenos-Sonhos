...
public class Tutorial_P5 : MonoBehaviour
{
	public GameObject Camera, playerActor, caixa, caixaDummy, Mao1, Mao2, RightBtn, LeftBtn, UpBtn, SwitchBtn, prevCutscene;
	...
	private IEnumerator Cutscene()
	{
		...
		playerActor.GetComponent<PlayerActingCommands>().PressRight(2);

		yield return new WaitForSeconds(0.6f);

		playerActor.GetComponent<PlayerActingCommands>().PressUp(0.5f);

		yield return new WaitForSeconds(1.4f);

		playerActor.GetComponent<PlayerActingCommands>().PressLeft(2.2f);
	...
	{
		if (!firstTouch && prevCutscene.GetComponent<Tutorial_P4>().sceneDone)
		{
